'''
code referred by https://github.com/Sekunde/3D-SIS/blob/375187f0d0b8474158121edda2d83fd07aa6bcf8/lib/utils/evaluation.py#L62
'''
import matplotlib
import numpy as np
import os
...
class DetectionMAP:
    ...
    def evaluate(self, pred_bb, gt_bb):
        """
    ...
    def compute_TP_FP_FN(IoU):
        ...
        TP = []
        FP = []
        FN = IoU.shape[1]  # n. of gt box #sum(gt_cls == class_index)
        ...
        for i in range(len(IoU)):
            ...
            for j in range(IoU_mask.shape[1]):
                if IoU_mask[i,j] == True and IoU[i,j] > max_overlapping:  # overlapped gt box 중 iou 가 제일 큰 것을 할당
                    ind = j
...
class APAccumulator:
    ...
    def __init__(self):
        ...
        # for the case that our method may generate not only gt boxes and more accurate another boxes
